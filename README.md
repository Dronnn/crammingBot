...
- `/fullword` (полная карточка по введенному слову: сначала из сохраненного snapshot; если snapshot отсутствует, создается и сохраняется; поиск игнорирует регистр/лишнюю пунктуацию)
- `/help`
- `/cancel`
