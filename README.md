...
- `/fullword` (полная карточка по введенному слову из данных в памяти/БД; без генерации; поиск игнорирует регистр/лишнюю пунктуацию)
- `/help`
- `/cancel`
...
- `/fullword` asks for a word and shows a full card only from stored DB data (snapshot if present, otherwise assembled from saved word/examples), with no LLM generation fallback and normalized lookup (case-insensitive, punctuation-tolerant, German article-tolerant).
